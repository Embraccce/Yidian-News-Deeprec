...
########################################################################
               #################定义模型##############
########################################################################
def DeepFM_lhuc(linear_feature_columns, dnn_feature_columns, lhuc_feature_columns, cate_map, fm_embed_dim=64, dnn_hidden_units=[512, 256, 128], dnn_activation='relu', seed=1024,):
    ...
    # LHUC part
    lhuc_embedding_dict = build_embedding_dict(lhuc_feature_columns, dim=fm_embed_dim, name_suf="_lhuc")
    lhuc_sparse_embedding_list, _ = input_from_feature_columns(features, lhuc_feature_columns, lhuc_embedding_dict, cate_map)
    lhuc_input = combined_dnn_input(lhuc_sparse_embedding_list, [])

    dnn_out = LHUC(
        hidden_units=dnn_hidden_units,
        lhuc_size=lhuc_input.shape[-1],
        dnn_activation=dnn_activation,
        name="personalized_lhuc"
    )(dnn_input, lhuc_input)
