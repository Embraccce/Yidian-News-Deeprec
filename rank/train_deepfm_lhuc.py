...
from models.deepfm_lhuc import DeepFM_lhuc
...
lhuc_feature_columns_name = config['model']['deepfm_lhuc']['lhuc_feature_columns_name']
...
lhuc_feature_columns  = [col for col in feature_columns if col.name in lhuc_feature_columns_name ]


# 初始化数据集处理器
dataset_creator = NewsDataset(config, feature_columns, model="deepfm_lhuc")
...
model = DeepFM_lhuc(linear_feature_columns, dnn_feature_columns, lhuc_feature_columns,
               cate_map=config['feature']['categorical_vocabs'],
               fm_embed_dim=config['model']['deepfm_lhuc']['fm_embed_dim'],
               dnn_hidden_units=config['model']['deepfm_lhuc']['dnn_hidden_units'],
               dnn_activation=config['model']['deepfm_lhuc']['dnn_activation'], 
               seed=config['model']['deepfm_lhuc']['seed'],
               )
...
# 保存模型
model.save(f"/data3/zxh/news_rec/temp_results/deepfm_lhuc/002/")  # 这会创建一个文件夹，包含 SavedModel 格式的模型
