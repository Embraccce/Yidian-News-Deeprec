...
class NewsDataset:
    def __init__(self, config: Dict, feature_columns, model="esmm"):
        ...
        # 需要分桶的参数
        self.bucket_dict = self.feature_cfg.get('bucket_config', None)
    ...
    def _load_pretrained_embeddings(self, ipc_file) -> Tuple[tf.lookup.StaticHashTable, tf.Tensor]:
        ...
        item_bert_embed = np.vstack([[0.0] * 64, item_bert_embed])
        # 转换为 TensorFlow 张量
    ...
    def _get_default_values(self, col_name, feature_columns) -> list:
        ...
        for feature_name in col_name:
            ...
            elif dtype in ['float32', 'float64'] or feature_name == "duration":
    ...
    def _get_pad_shapes_and_values(self, feature_columns) -> Tuple[tf.Tensor, tf.Tensor]:
        ...
        # no need to pad labels 
                ...
                pad_values[feat_col.name] = 0.0 if feat_col.dtype == 'float32' else 0
        if self.model == "essm":
            pad_shapes = (pad_shapes, (tf.TensorShape([]), tf.TensorShape([])))
            pad_values = (pad_values, (tf.constant(0, dtype=tf.int32), tf.constant(0, dtype=tf.int32)))
        else:   
    ...
    def _parse_function(self, example_proto: tf.Tensor) -> Tuple[Dict, tf.Tensor]:
        ...
        ctr_label = parsed['is_clicked']
        cvr_label = parsed['is_converted']
        if self.model == "essm":
            return feature_dict, (ctr_label, cvr_label)
        return feature_dict, ctr_label
    ...
    def create_dataset(self, data_path: str, shuffle: bool = False) -> tf.data.Dataset:
        ...
        if shuffle:
            dataset = dataset.shuffle(
                buffer_size=self.train_cfg['global'].get('shuffle_buffer_size', 1000000),
